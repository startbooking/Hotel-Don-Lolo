<?php 
  ...
  $pdf->Cell(40,5,str_pad($nrocongela,5,'0',STR_PAD_LEFT),1,1,'C');
  $pdf->Ln(3);
  ...
  if($idcentro!=0){
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(150,5,utf8_decode($datosCompania[0]['empresa']),0,1,'L');
    $pdf->SetFont('Arial','',10);
...
*/
  ...
  $pdf->SetFont('Arial','B',10);
  $pdf->Cell(70,5,utf8_decode($datosHuesped[0]["apellidos"].' '.$datosHuesped[0]["nombres"]),0,0,'L');
  $pdf->SetFont('Arial','',10);
  ...
  */
  ...
  /*
  $pdf->SetY(230);
  $pdf->Rect(10, 230, 190, 19);
  $pdf->Cell(30,5,'Datos Huesped',0,1,'R');
  $pdf->Ln(8);
  $pdf->Cell(60,5,'Nombres y Apellidos',0,0,'C');
  $pdf->Cell(60,5,'Identificacion',0,0,'C');
  $pdf->Cell(70,5,'Firma',0,1,'C');
  */
...
Firma                                              Fecha',1,'L');
  $pdf->Ln(1);
  $pdf->MultiCell(190,4,utf8_decode('Acepto los consumos de Estado de Cuenta, autorizo con mi firma en la presente estado de cuenta y la generacion de la factura de cobro sin la misma, segun el contrato establecido con la empresa.'),0,'C');



  $file = 'congela/Cuenta_Congelada_'.$nrocongela.'.pdf';
