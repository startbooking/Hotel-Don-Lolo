...
function agrega_a_lista(){
	...
	vali = (valp * (pori /100));
	vato = vali + (valu*cant);
  $("#tabla_articulos > tbody").append("<tr><td>"+prod+"</td><td>"+desc+"</td><td align='right'>"+cant+"</td><td align='right'>"+valu.toFixed(2)+"</td><td align='right'>"+vali.toFixed(2)+"</td><td align='right'>"+vato.toFixed(2)+"</td><td align='center'><button id='"+prod+"' class='btn btn-danger btn-xs elimina_articulo' onclick='actualiza_entrada_temp(this.id);'><i class='fa fa-times'></i></button></td></tr>");
    ...
    $.ajax({
      ...
      type: 'POST',
      data: "tipo="+'1'+"&tipo_movi="+$("#tipo_movi").val()+"&factura="+$("#factura").val()+
      "&fecha="+$("#fecha").val()+"&proveedor="+$("#proveedor").val()+"&producto="+prod+
      "&cantidad="+cant+"&unidad="+unid+"&valoruni="+valu+"&valortot="+valp+"&impuesto="+
      vali+"&nombre="+desc+"&almacen="+alma+"&porc_impu="+pori+"&cod_impu="+impu,
      success: function(z){
        if(z=="0"){
      	...
      	{addClass: 'btn btn-primary',
            ...
            $('#tabla_articulos > tbody > tr').each(function(){
              ...
              $.ajax({
                ...
                type: 'POST',
                data: "tipo="+'1'+"&tipo_movi="+$("#tipo_movi").val()+"&factura="+$("#factura").val()+
                "&fecha="+$("#fecha").val()+"&proveedor="+$("#proveedor").val()+"&producto="+codi+
