<?php 
 ...
 /*  $pagosCaj           = $pos->getDetalleFormasdePagoCajero('A',$user, $idamb);
  ...
  $abonosCaj   = $pos->getDetalleAbonosFormasdePago($idamb);
  $pagosCaj    = $pos->getDetalleFormasdePagoCajero('A',$user, $idamb);
  ...
  if(count($carteraCaj)==0){
    ...
    $pdf->SetFont('Arial','',10);
    $pdf->Cell(70,6,'Concepto ',0,0,'C');
    $pdf->Cell(70,6,'Proveedor. ',0,0,'C');
    $pdf->Cell(20,6,'Valor Pagado',0,0,'C');
    $pdf->Cell(20,6,'',0,1,'C');
    ...
    foreach ($carteraCaj as $caja) {
      ...
      $pdf->Cell(20,5,number_format($caja['monto'],2),0,0,'R');
      $pdf->Cell(20,5,'',0,1,'R');
    }
  }
  $pdf->Ln(2);
 ...
 */
  $pdf->Ln(3);
  $file = '../../../imprimir/cierres/balance_Cajero_'.$user.'_'.$fecha.'.pdf';
  $pdf->Output($file,'F');
