<?php 
  ...
  require '../../../../res/fpdf/fpdf.php';
	...
	/// $pref = $pos->prefijoResolucion($amb);
	$pref     = $pos->getPrefijoAmbiente($amb);
	$docu     = $pref.$nFact;
	$tipo     = 2;

	$tipMovi       = $pos->traeDatosMovimiento();
	$movimiento     = $tipMovi[0]['tipo']; 
	$tipoMovi       = $tipMovi[0]['id_tipomovi'];
	...
	if(count($productosVenta)==0 && count($recetasVenta)==0){
		...
		if(count($productosVenta)>0){
			foreach ($productosVenta as $key => $prodSale) {
				...
				$adiPro = $pos->insertaMovimiento($tipoMovi, $tipo, $movimiento, $numeroMov, $fecha, $centroCosto, $docu, $producto, $cantidad, $unidadalm, $unit, $subtotal, $costo, $almacen, 1, $usuario);
			}
		}
