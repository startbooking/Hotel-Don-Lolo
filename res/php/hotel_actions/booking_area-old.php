<?php   
  ...
  $ninos       = $_POST['ninos']; 
  $noches      = $_POST['noches'];
  $fechainuni  = strtotime($fechain.' '.CHECKIN.':00') ; 
  $fechaoutuni = strtotime(($fechaout).' '.CHECKIN.':00'); 
  ...
  foreach($hoteles as $hotel): 
    ...
    <div class="row-fluid" style='border-radius:5px;border:1px solid #A49C9C;padding:5px'>
      <?php 
        $rooms = $user->getRoomHotel($hotel['id_hotel']);
        foreach($rooms as $room): ?>
          <div class="row" >
            <div class="col-lg-4 col-md-4 col-xs-6" style='margin-top:15px;padding-right: 10px;padding-left: 0px;'>
              <img class="thumbnail" src="<?php echo BASE_IMAGES ?>rooms/<?=$room['image']?>" alt="" style='height: 100%;width: 100%;'>
            </div>
            <div class="col-lg-5 col-md-5 col-xs-6" style='margin-top:15px'>
              <div class="row">
                <div class="row">
                  <label for=""><?=$room['room_name']?></label><br>
                  <label for="" style="font-size:0.9em"><?=$room['sub_text']?></label><br>
                  <div class="row">
                  ...
                  <div class="row row-facility" style="margin-top:5px">
                    <?php 
                      ...
                      foreach($facilidades as $facilidad): ?>
                        <div class="facility">
                          <img class="img-thumbnail" src="<?php echo BASE_IMAGES ?>facility/<?=$facilidad['image']?>" alt="<?=$facilidad['description']?>" title="<?=$facilidad['description']?>" style='width: 85%;'>  
                        </div>
                      <?php endforeach; 
                    ?>
                  </div>
            ...
            <div class="col-lg-3 col-md-3 col-xs-12" style='margin-top:15px;padding: 0'>
              <?php
                if($activo==2){?>
                  <h2 class="alert alert-danger" style="color:brown;font-weight: 600;margin-top:25px;padding:10px;width: 220px">Sin Disponibilidad 01</h2>
                  <?php 
                }else{
                  $botonreserva = "SI";                
                  ...
                  if($numreg==0){   ?>
                    <div class='alert alert-danger diagonal' style='color:#620808;font-size:0.8em;font-weight:600;text-transform: uppercase;'>
                      <center>Sin Disponibilidad 02</center>
                    </div>
                    <?php 
                  }else{ ?>
