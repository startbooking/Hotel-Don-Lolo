<script src="//cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.js"></script>
<script src="https://code.jquery.com/jquery-2.2.4.js" integrity="sha256-iT6Q9iMJYuQiMWNd9lDyBUStIq/8PuOW33aOqmvFpqI=" crossorigin="anonymous">
</script>

<!-- <script src="<?= BASE_RES ?>dist/bootstrap.min.js"></script> -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@3.3.7/dist/js/bootstrap.min.js"></script>
<!-- <script src="<?= BASE_JS ?>pdf-lib.js" type="text/javascript"></script> -->
<!-- <script src="https://unpkg.com/pdf-lib@1.4.0"></script> -->
<script src="https://cdn.datatables.net/1.13.7/js/jquery.dataTables.min.js"></script>
<script src="<?= BASE_JS ?>tomarfoto.js" type="text/javascript"></script>
<script src="<?= BASE_JS ?>sactel.js" type="text/javascript"></script>
<script src="<?= BASE_CSS ?>sweetalert.min.js" type="text/javascript" charset="utf-8" async defer></script>
<script src="<?= BASE_RES ?>dist/app.min.js"></script>
<!-- <script src="<?= BASE_RES ?>dist/jquery.dataTables.min.js"></script> -->