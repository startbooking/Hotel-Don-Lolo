...
if (count($reservas) >= 1) { ?>
  ...
  if ($regis > 0) { ?>
    <div class="container-fluid">
      <div class="alert alert-warning" style="padding:0;padding-top:5px;">
        <h4 align="center" style="font-weight: 600;">Actualize los Datos del Cliente e Imprima los Registros Hoteleroaa </h4>
      </div>
      <div class="table-responsive">
        <table id="example1" class="table table-bordered">
          <thead>
  ...
  <?php
    ...
    foreach ($procesos as $proceso) {
      ...
      <?php
        ...
        <script>
          $('#mensajeAuditoria').html('');
          $('#mensajeAuditoria').html('<h4 class="bg-red" style="padding:10px">Proceso : <span style="font-size:24px;font-weight: 700;font-family: '
            ubuntu '"><?= $proceso['titulo_proceso'] ?></span> Ya Ejecutado</h4>');
        </script>
    <?php
      }
