...
$nombreExp = $hotel->getNombreCiudad($huesped['ciudad_expedicion']);
$nombreCiu = $hotel->getNombreCiudad($huesped['ciudad']);
...
<form class="form-horizontal" id="formUpdateHuesped" action="javascript:actualizaHuesped()" method="POST">
  <div class="form-group">
    ...
    <div class="col-sm-3">
      <select name="tipodoc" required value="<?php echo $huesped['tipo_identifica']; ?>">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
      <select name="paisExpUpd" id="paisExpUpd" required="" onblur="ciudadesExpedicion(this.value,'<?php echo $huesped['ciudad_expedicion']; ?>')">
        <?php
  ...
  <div class="form-group">
    ...
    <div class="col-sm-2" style="padding:0">
      ...
      <div class="col-sm-6" style="padding:0;height: 15px">
        <div class="form-check form-check-inline" style="text-align: left">
          <input style="margin-top:5px" class="form-check-input" type="radio" name="sexOption" id="inlineRadio2" value="2" <?php if ($huesped['sexo'] == 2) { ?> checked="" <?php } ?>>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-4">
      <select name="paices" id="paices" onblur="getCiudadesPais(this.value,'<?php echo $huesped['ciudad']; ?>')">
    ...
    <div class="col-sm-4">
      <select name="ciudadUpd" id='ciudadUpd'>
        <option value="<?php echo $huesped['ciudad']; ?>"><?php echo $nombreCiu; ?></option>
      </select>
    </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-4">
      <input type="text" class="form-control" name="telefono" id="telefono" value="<?php echo $huesped['telefono']; ?>" required="" minlength="10" maxlength="18" pattern="[0-9]+">
    </div>
    <label for="celular" class="col-sm-2 control-label">Celular</label>
    <div class="col-sm-4">
      <input type="text" class="form-control" name="celular" id="celular" value="<?php echo $huesped['celular']; ?>" minlength="10" maxlength="18" pattern="[0-9]+">
    </div>
  </div>
  <div class="form-group">
    <label for="correo" class="col-sm-2 control-label">Correo </label>
    <div class="col-sm-4">
      <input type="email" class="form-control" name="correo" id="correo" value="<?php echo $huesped['email']; ?>" required>
    </div>
    <label for="fechanace" class="col-sm-2 control-label">Fecha Nacimiento </label>
    <div class="col-sm-4">
      <input type="date" class="form-control" name="fechanace" id="fechanace" value="<?php echo $huesped['fecha_nacimiento']; ?>">
    </div>
  </div>
  <div class="form-group">
      ...
      <select name="tipoAdquiriente" id="tipoAdquiriente" required>
        ...
        foreach ($tipoAdquiere as $tipoAdqui) { ?>
          <option value="<?php echo $tipoAdqui['id']; ?>"
            <?php
            if ($huesped['tipoAdquiriente'] == $tipoAdqui['id']) { ?> selected <?php  } ?>>
      ...
      <select name="tipoResponsabilidad" id="tipoResponsabilidad" required>
        ...
        foreach ($tipoRespo as $tipoRes) { ?>
          <option value="<?php echo $tipoRes['id']; ?>" <?php
                                                        if ($huesped['tipoResponsabilidad'] == $tipoRes['id']) { ?> selected <?php } ?>>
    ...
    <div class="col-sm-4">
      <select name="responsabilidadTribu" id="responsabilidadTribu" required>
        ...
        foreach ($tipoTribus as $tipoTribu) { ?>
          <option value="<?php echo $tipoTribu['id']; ?>" <?php if ($huesped['responsabilidadTributaria'] == $tipoTribu['id']) { ?> selected <?php } ?>><?php echo $tipoTribu['descripcionResponsabilidad']; ?></option>
        <?php
        }
        ?>
      </select>
      ...
      <select name="profesion" id="profesion">
        ...
        foreach ($motivos as $motivo) { ?>
          <option value="<?php echo $motivo['id_grupo']; ?>" <?php
                                                              if ($motivo['id_grupo'] == $huesped['profesion']) { ?> selected <?php } ?>>
            <?php echo $motivo['descripcion_grupo']; ?>
          </option>
        <?php
        }
        ?>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-4">
      <select name="tarifaUpd" id="tarifaUpd" required="">
