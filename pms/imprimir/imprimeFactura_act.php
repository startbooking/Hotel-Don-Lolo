<?php 
  ...
  $pdf->Cell(190,5,'NO SOMOS GRANDES CONTRIBUYENTES NI AGENTES RETENEDORES',0,1,'C');
  ...
  $pdf->Cell(40,5,'FACTURA DE VENTA ',1,1,'C');
  $pdf->Cell(40,5,'',0,0,'C');
  ...
  if($tipofac==2){
    if(!empty($datosCompania)){ 
      $pdf->SetFont('Arial','',10);
      if($idcentro!=0){
        $pdf->Cell(40,5,'RAZON SOCIAL',0,0,'L');
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(160,5,utf8_decode($datosCompania[0]['empresa']),0,1,'L');
        $pdf->SetFont('Arial','',10);
        $pdf->Cell(40,5,'CENTRO DE COSTO',0,0,'L');
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(70,5,utf8_decode($datosCentroCia[0]['descripcion_centro']),0,0,'L');        
        $pdf->SetFont('Arial','',10);
    ...
    $pdf->Cell(30,5,'DIRECCION',0,0,'L');
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(70,5,substr(utf8_decode($datosHuesped[0]['direccion']),30),0,0,'L');
    $pdf->SetFont('Arial','',10);
  ...
  $pdf->SetFont('Arial','B',10);
  $pdf->Cell(70,5,substr(utf8_decode($datosHuesped[0]["nombre_completo"]),0,30),0,0,'L');
  $pdf->SetFont('Arial','',10);
  ...
  $pdf->Cell(47,5,$fechaFac,1,0,'C');
  $pdf->Cell(48,5,$fechaFac,1,0,'C');
  ...
  $pdf->Ln(3);
  $pdf->SetFont('Arial','B',10);
  $pdf->Cell(100,5,'IMPUESTOS',1,1,'C');
  $pdf->SetFont('Arial','',10);
  $pdf->Cell(40,5,'TIPO IMPUESTO',1,0,'C');
  $pdf->Cell(30,5,'BASE',1,0,'C');
  $pdf->Cell(30,5,'VALOR',1,1,'C');
  foreach ($tipoimptos as $tipoimpto) {  
    $pdf->Cell(40,5,$tipoimpto['descripcion_cargo'],1,0,'L');
  ...
  $pdf->SetX(110);
  $pdf->SetFont('Arial','B',10);
  $pdf->Cell(90,5,'FORMAS DE PAGO ',1,1,'C');
  $pdf->SetX(110);
  $pdf->Cell(60,5,'DETALLE',1,0,'C');
  $pdf->Cell(30,5,'VALOR',1,1,'R');
  $pagos    = 0;
  $pdf->SetFont('Arial','',10);
  foreach ($pagosfolio as $pagofolio) {  
    $pdf->SetX(110);
    $pagos    = $pagos + $pagofolio['pagos']; 
    $pdf->Cell(60,5,$pagofolio['descripcion_cargo'],1,0,'L');
    $pdf->Cell(30,5,number_format($pagofolio['pagos'],2),1,1,'R');
  }
  $pdf->Cell(100,5,'',0,0,'L');
  $pdf->SetFont('Arial','B',11);
  $pdf->Cell(60,6,'TOTAL ',1,0,'C');
  $pdf->Cell(30,6,number_format($pagos,2),1,1,'R');
  $pdf->SetFont('Arial','',7);
  $y = $pdf->GetY();
  $pdf->SetY(214);
  $pdf->MultiCell(190,4,'FACTURA IMPRESA POR COMPUTADOR. Resolucion Nro 18762013314547 del 2019-03-06 Autoriza Prefijo H desde el No 0001 al No 2000, Esta Factura de venta se asimila en todos sus efectos a una letra de cambio Art. 774 del Codigo de Comercio, Actvidad Economica Principal 5511. Categoria de Sociedad: Microempresa - Tarifa Renta 2017-2021 : 0% - FAVOR NO APLICAR RETENCION EN LA FUENTE',1,'C');
  $pdf->SetY(226);
  $pdf->MultiCell(95,4,'
