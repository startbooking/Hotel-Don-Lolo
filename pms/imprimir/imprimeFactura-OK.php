<?php 
  ...
  $folios     = $hotel->getConsumosReservaAgrupadoCodigoFolio($reserva,$nroFolio,1);
  $pagosfolio = $hotel->getConsumosReservaAgrupadoCodigoFolio($reserva,$nroFolio,3);
  $tipoimptos = $hotel->getValorImptoFolio($reserva,$nroFolio,2);
  ...
  if(!empty($datosReserva[0]['id_compania'])){
    if(empty($datosCompania)){ 
      ...
      $pdf->SetFont('Arial','B',10);
      $pdf->Cell(60,5,utf8_decode($datosCompania[0]['empresa']),0,0,'L');
      $pdf->SetFont('Arial','',10);
      ...
      $pdf->SetFont('Arial','B',10);
      $pdf->Cell(50,5,utf8_decode($datosCompania[0]['direccion']),0,0,'L');
      $pdf->SetFont('Arial','',10);
      $pdf->Cell(20,5,'CIUDAD',0,0,'L');
      $pdf->SetFont('Arial','B',10);
      $pdf->Cell(30,5,$datosCompania[0]['ciudad'],0,0,'L');
      $pdf->SetFont('Arial','',10);
    ...
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(80,5,utf8_decode($datosHuesped[0]["apellidos"].' '.$datosHuesped[0]["nombres"]),0,0,'L');
    $pdf->SetFont('Arial','',10);
    ...
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(10,5,$datosHuesped[0]['ciudad'],0,0,'L');
    $pdf->SetFont('Arial','',10);
  ...
  $pdf->Cell(48,5,$datosReserva[0]["fecha_salida"],1,0,'C');
  ...
  $pdf->setY(225);
  $pdf->SetFont('Arial','',7);
  $pdf->MultiCell(95,4,'Favor Consignar a la Cuenta Corriente Nro 1369 00089 915 del Banco DAVIVIENDA a nombre de TORZAL SAS',1,'C');
  $y = $pdf->GetY();
  $pdf->SetY(225);
  $pdf->SetX(105);
  $pdf->MultiCell(95,6,'  
  ...
  // $pdf->Cell(19,6,'',0,1,'L');
  $pdf->SetY(233);
  $pdf->MultiCell(95,4,'FACTURA IMPRESA POR COMPUTADOR. Resolucion Nro 18762003822008 del 2017-06-28 Autorizacion desde el No 1 al No 5063, Esta Factura de venta se asimila en todos sus efectos a una leta de cambio Art. 774 del Codigo de Comercio, Actvidad Economica Principal 5513 tarifa 4.00 x 1000',1,'C');
  ...
  // $file = 'Factura_Nro_'.str_pad($nroFactura,5,'0',STR_PAD_LEFT).'.pdf';
