<?php 
  // require 'plantillaAuditoriaHor.php';
  ...
  if($regis==0){
    ...
    foreach ($reservas as $reserva) {
      ...
      $pdf->Cell(80,4,($reserva['apellido1'].' '.$reserva['apellido2'].' '.$reserva['nombre1'].' '.$reserva['nombre2']),0,0,'L');
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(30,5,'Consumos',1,0,'C');
    $pdf->Cell(30,5,number_format($car,2),1,0,'R');
    $pdf->Cell(30,5,'Impuesto',1,0,'C');
    $pdf->Cell(30,5,number_format($imp,2),1,0,'R');
    $pdf->Cell(30,5,'Abonos',1,0,'C');
    $pdf->Cell(40,5,number_format($pag,2),1,0,'R');
    $pdf->Cell(30,5,'Saldo',1,0,'C');
    $pdf->Cell(40,5,number_format($tot,2),1,1,'R');
 
  }

  $file = '../../imprimir/auditorias/Balance_huesped_'.FECHA_PMS.'.pdf';
  $pdf->Output($file,'F');
?>
