<?php 
  ...
  $anio = substr($fechaBus,0,4); 
  $mes  = substr($fechaBus,5,2); 
  ...
  $pdf->Cell(260,4,'Fecha : '.$fechaBus,0,1,'C');
  $pdf->Ln(4);
  $pdf->SetFont('Arial','',9);
  ...
  foreach ($codigos as $codigo) { 
    ...
    if(count($diavta)==0){$cargos = 0 ;}else{$cargos = $diavta[0]['cargos']; }
  ...
  $file = '../../imprimir/cajeros/Balance_DiarioAcumulado_'.$fechaBus.'.pdf';
  $pdf->Output($file,'F');
?>
